...
static const struct shell_cmd_help set_help = {
   ...
   .usage = "\tset [direction] [hexa of packet ll pdu] [hexa of new packet ll pdu] [hexa of the value]\n\tset mac_target [mac adress in hex with this format XX:XX:XX:XX:XX:XX]\n\tset mac_target 0 to delete it",
   .params = NULL,
};
static struct shell_cmd shell_set_cmd_struct = {
    .sc_cmd = "set",
...
//    int nb_of_modified_packets = sizeof(list_mitmed_packet);
    ...
    if(argc == 3){
        ...
        console_printf("atoi(argv[2]) : %x\n", atoi(argv[2]));
        if(!(strcmp(argv[1], "feat")) )
        {
            console_printf("\nWill set the features to %i\n", atoi(argv[2]) );
            mitm_params_local_features = atoi(argv[2]);
        }
        // exemple of a the command : set mac_target c9:d9:c6:07:85:61
        // to delete the target address you can use the command set mac_target 0
        else if (!(strcmp(argv[1], "mac_target")) )
        {
            console_printf("going to store this mac address of the target: %s arg size %i\n", argv[2], strlen(argv[2]));
            //search for 0 or c9:d9:c6:07:85:61
            if( strlen(argv[2]) == 1)
            {
                console_printf("disabling target mac\n");
                target_is_set = false;
            }
            else if (strlen(argv[2]) == 17)
            {
                // because mac is stored lsb
                int decreaser_to_store_mac = 5;
                char testchar[2];
                for( int uhgg = 0; uhgg< strlen(argv[2]); uhgg++)
                {
                    if(argv[2][uhgg] == ':')
                    {
                        console_printf("%c\n", argv[2][uhgg] ) ;
                    ...
                    {
                        console_printf("%c%c\n", argv[2][uhgg],argv[2][uhgg+1] );
                        testchar[0] = argv[2][uhgg];
                        testchar[1] =  argv[2][uhgg+1];
                        target_of_masterer[decreaser_to_store_mac] = (uint8_t)strtol(testchar, NULL, 16);



                        decreaser_to_store_mac-=1;
                        uhgg+=1;
                    }
                }
                //target_is_set = true;
            ...
            //target_is_set = true or false
            //target_of_masterer
        }
        else
        {
            console_printf("Wrong args\n");
        }
